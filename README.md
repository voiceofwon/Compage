...
* 1.2.1 방문자수 monitoring system
* 1.2.2 게시판 목록 접속시 발생하는 N+1 problem fix(fetch join으로 한번에 조회)
  * (findAll 메서드 호출 시 게시글 첨부파일(하위 엔티티)을 각각의 게시글에 대하여 N개의 쿼리가 추가적으로 발생)
...
반응시간이 특히 길어지는 시간대를 확인 해보니, 학교 수업의 출결인증 시간대와 겹치는 것을 확인하였습니다.

실제로 수강신청 시 학교 인터넷 망에 트래픽이 몰리며 서버가 다운됐을 때, 제가 운영 중인 사이트가 같이 다운되는 것을 확인하였습니다.

학교 인터넷 설비 및 네트워크 망에 발생한 트래픽이 학교 내부 네트워크망에 배포한 제 사이트가 영향을 받는 것으로 판단하였습니다.

학교 네트워크에 QoS나 대기열이 형성되어 패킷 처리가 지연되는 것을 원인으로 분석하였고, 서버 어플리케이션 상의 문제가 아님을 확인하였습니다.

## <b>Visitor 집계 오류</b>

Visitor 집계는 사이트 접속시 userIP, userAgent, Date를 InMemory DB인 Redis에 임시로 저장해놨다가

한 시간에 한번식 동작하도록 스케쥴된 서비스를 통해 디스크 DB로 옮겨주는 작업을 진행하도록 구현하였습니다. 하지만 특정 접속 정보에 대해서 redis에서 mysql로 옮겨지지 않는 문제가 발생하였습니다.

<대처>

sql 로그를 확인해 보니, 특정 userAgent에 대하여 설정해놓은 String 최대길이를 초과하여 redis에서 기존 데이터를 삭제하고 mysql로 옮기는 기능이 동작하지 않았음을 확인하였습니다.

String 최대길이를 늘리거나 원하는 정보만을 파싱하여 저장하는 방법을 생각하였으나, 단순 방문자 수만 집계하는 서비스가 목적이었으므로 userAgent는 저장하지 않도록 서비스를 수정하여 해결하였습니다.

## <b>N+1 Problem</b>

모든 게시판의 목록 page 접속시 findAll 메서드 호출이 발생한는데, 이때 게시글 첨부파일(하위 엔티티)을 조회하며 각각의 게시글의 첨부파일에 대하여 N개의 쿼리가 추가적으로 발생함을 확인하였습니다.

<대처>

불필요한 쿼리가 발생함을 확인하여 첨부파일까지 하나의 쿼리로 조회될 수 있도록 fetch join을 활용하여 findAll method에 대하여 의도치않은 query가 추가적으로 발생하지 않도록 수정하였습니다.
